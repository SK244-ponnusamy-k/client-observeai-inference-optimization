#!/bin/bash
# ==============================================================================
# vllm/models/gemma-4-31b/deploy.sh
#
# All model-specific names come from model.env — this script is generic.
#
# Usage:
#   cd llm-inference-framework
#   bash vllm/models/gemma-4-31b/deploy.sh
#   bash vllm/models/gemma-4-31b/deploy.sh --validate
# ==============================================================================
...
echo ""
echo "══════════════════════════════════════════════════"
echo "  DEPLOYING — ${MODEL_HF_ID} (instruction-tuned)"
echo "  Deployment: ${DEPLOYMENT_NAME}"
echo "  Service   : ${SERVICE_NAME}:8000"
echo "  Namespace : ${BENCHMARK_NAMESPACE}"
echo "  GPUs      : 4x L40S (g6e.12xlarge), tensor-parallel-size=4"
echo "══════════════════════════════════════════════════"
echo ""
...
if ! aws s3 ls "s3://${MODEL_BUCKET}/${MODEL_FOLDER}/" --region "${AWS_REGION}" >/dev/null 2>&1; then
    ...
    log_error "Download it first: bash model-download/gemma-4-31b/download.sh"
    exit 1
fi
...
# Wait for pod
log_info "Waiting for pod (Karpenter provisioning g6e.12xlarge GPU node — larger node, ~5-8 min)..."
sleep 20
...
# Wait for Ready — TP=4 rank sync + ~63GB streamed from S3 takes longer than single-GPU models
log_info "Waiting for pod to be Ready (model loading + TP=4 rank sync — up to 20 min)..."
