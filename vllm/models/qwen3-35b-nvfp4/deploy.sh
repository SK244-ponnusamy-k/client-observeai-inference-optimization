#!/bin/bash
# ==============================================================================
# vllm/models/qwen3-35b-nvfp4/deploy.sh
#
# All model-specific names come from model.env — this script is generic.
#
# Usage:
#   cd llm-inference-framework
#   bash vllm/models/qwen3-35b-nvfp4/deploy.sh
#   bash vllm/models/qwen3-35b-nvfp4/deploy.sh --validate
# ==============================================================================
...
if ! aws s3 ls "s3://${MODEL_BUCKET}/${MODEL_FOLDER}/" --region "${AWS_REGION}" >/dev/null 2>&1; then
    ...
    log_error "Download it first: bash model-download/qwen3-35b-nvfp4/download.sh"
    exit 1
fi
...
# Wait for pod
log_info "Waiting for pod (Karpenter provisioning g6e.12xlarge GPU node ~3-5 min)..."
sleep 15
...
# Wait for Ready — TP=2 + NVFP4 kernel compilation takes longer than single-GPU
log_info "Waiting for pod to be Ready (model loading + NVFP4 kernel compile — up to 15 min)..."
